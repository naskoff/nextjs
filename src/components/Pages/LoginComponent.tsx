...
export default function LoginComponent() {
    const [state, action, loading] = useActionState(SignInAction, {});
    ...
    return (
        <div className="max-w-[480px] w-full">
            <div className="p-6 sm:p-8 rounded-2xl bg-white border border-gray-200 shadow-sm">
                ...
                <form className="mt-12 space-y-6" action={action}>
                    <div>
                        <label className="text-slate-900 text-sm font-medium mb-2 block">User name</label>
                        <div className="relative flex items-center">
                            <input name="username" type="text" required
                                   className={`w-full text-slate-900 text-sm border border-slate-300 px-4 py-3 pr-8 rounded-md outline-blue-600 ${state?.errors?.fields?.username ? 'border border-red-500 text-red-900' : ''}`}
